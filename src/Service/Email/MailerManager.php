...
final class MailerManager
{
    ...
    {
        $email = (new TemplatedEmail())
            ->from(new Address($from, 'Philip Frères'))
            ->to($to)
            ->subject($subject)
            ->htmlTemplate($template)
            ->context($context)
    ...
    /**
     * Mail dédié à la création de compte (TENANT_EMPLOYE ou formateur)
    ...
    public function sendNewAccountEmail(
        ...
        $subject = ($isFormateur ? 'Votre compte formateur' : 'Votre compte TENANT_EMPLOYE')
            . ' sur ' . $entite->getNom();
    ...
    public function sendHtml(
        ...
        $from = $fromOverride
            ? new Address($fromOverride, $this->resolveFromAddress($entite)->getName() ?: 'Philip Frères')
    ...
    public function sendAccountCreatedValidation(Utilisateur $user, Entite $entite): void
    {
        ...
        $subject = 'Votre compte vient d’être créé - ' . ($entite->getNom() ?? 'Philip Frères');
